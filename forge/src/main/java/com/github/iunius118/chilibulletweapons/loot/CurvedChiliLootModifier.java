...
/**
 * Loot Modifier to add curved chili peppers to the grass loot table in the Forge version.
 */
public class CurvedChiliLootModifier extends LootModifier {
    public static final Codec<CurvedChiliLootModifier> CODEC = RecordCodecBuilder.create(
            inst -> codecStart(inst)
                    .and(Codec.INT.fieldOf("count").forGetter(m -> m.bonusCount))
                    .apply(inst, CurvedChiliLootModifier::new));
