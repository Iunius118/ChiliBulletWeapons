...
public class HotSauce extends Item {
    ...
    public HotSauce(Properties properties, int effectDuration, int color) {
        super(properties);
        this.color = color;
        this.effectDuration = effectDuration;

        // Register dispense behavior
        DispenserBlock.registerBehavior(this, new HotSauce.DispenseBehavior(this));
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            ...
            // Play sound effect
            level.playSound(null, thrownpotion.getX(), thrownpotion.getY(), thrownpotion.getZ(), SoundEvents.SPLASH_POTION_THROW, SoundSource.PLAYERS, 0.5F, 0.4F / (level.getRandom().nextFloat() * 0.4F + 0.8F));
        }
    ...
    public static class DispenseBehavior extends AbstractProjectileDispenseBehavior {
        private final HotSauce hotSauce;

        public DispenseBehavior(HotSauce hotSauce) {
